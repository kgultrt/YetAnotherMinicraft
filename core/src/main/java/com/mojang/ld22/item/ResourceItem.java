...
import com.mojang.ld22.save.ResourceCodec;
...
public class ResourceItem extends Item {
    ...
    public void renderInventory(Screen screen, int x, int y) {
        screen.render(x, y, resource.sprite, resource.color, 0);
        Font.draw(resource.getDisplayName(), screen, x + 32, y, Color.get(-1, 555, 555, 555));
        int cc = count;
        if (cc > 999) cc = 999;
    ...
    public void read(DataInputStream in) throws IOException {
        super.read(in);
        resource = ResourceCodec.byKey(in.readUTF());
        count = in.readInt();
    }
}